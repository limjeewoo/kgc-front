...
// ✅ '교수 등록'을 NOT_IMPLEMENTED에서 제거 (실제 컴포넌트 연결)
const NOT_IMPLEMENTED = new Set(['마일리지 승인', '상담 내역']);
...
export default function AdminDashboard() {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        try {
          ...
          console.warn('⚠️ 학과 정보를 불러오지 못했습니다.');
        }
        ...
        if (studentsRes.status === 'fulfilled' && studentsRes.value?.data?.success) {
          ...
          setTotalStudents(students.filter(s => s.enrollStatus === '등록').length);
        }
  ...
  const handleMenuClick = (menuName) => {
    ...
    if (!SEARCH_SUB_MENUS.includes(menuName)) {
      setSearchDropdownOpen(false);
    }
    if (!PROF_SUB_MENUS.includes(menuName) && menuName !== '교수 등록') {
      setProfDropdownOpen(false);
    }
  };
      ...
      <div className="admin-wrap">
        ...
        <div className="main">
          ...
          <div className="content">
              ...
              <>
                ...
                <div className="bottom-grid">
                  <div className="card">
                    ...
                    {visaList.length === 0 ? <div className="empty-state">만료 임박 비자가 없습니다.</div> : 
                      visaList.map(v => (
                  ...
                  <div className="card">
                    ...
                    {attendanceList.length === 0 ? <div className="empty-state">출결 위험군 학생이 없습니다.</div> : 
                ...
                <div className="card">
                  ...
                  {onlineList.length === 0 ? <div className="empty-state">초과 학생이 없습니다.</div> : 
                    onlineList.map(o => (
                      <div key={o.studentId} className="online-row">
                        <div style={{width:'80px', fontSize:'0.81rem', fontWeight:'600'}}>{o.korName}</div>
                        <div className="progress-bar"><div className="progress-fill" style={{width:`${Math.min(o.onlineRatio * 100, 100)}%`}} /></div>
                        <div style={{width:'50px', fontSize:'0.75rem', fontWeight:'bold', color:'#EF4444', textAlign:'right'}}>{(o.onlineRatio * 100).toFixed(1)}%</div>
                      </div>
                    ))
                  }
                </div>
            ...
            {/* 교수 관리 라우팅 수정 */}
            {activeMenu === '전체 교수 목록' && (
              <ProfessorList onRegisterClick={() => setActiveMenu('교수 등록')} />
            )}

            {/* 지도교수 배정 관리 연결 */}
            {activeMenu === '학생-지도교수 배정 관리' && (<AdvisorAssign />)}
            
            {/* 교수 등록 컴포넌트 실제 연결 (미구현 fallback에서 빠짐) */}
            {activeMenu === '교수 등록' && (
              <ProfessorRegister 
                onComplete={() => setActiveMenu('전체 교수 목록')} 
                onCancel={() => setActiveMenu('전체 교수 목록')} 
              />
            )}

            {/* 미구현 fallback (교수 등록 폼 제외됨) */}
            {NOT_IMPLEMENTED.has(activeMenu) && (
              <div className="not-impl">
                <h2>{activeMenu} 페이지</h2>
                <p>이곳에 {activeMenu} 관련 기능을 구현해 주세요.</p>
              </div>
            )}
          </div>
        </div>
