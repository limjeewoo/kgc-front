...
export default function VisaTab() {
    ...
    setTimeout(() => {
      setVisaData({
        currentVisa: {
          ...
          dDay: 45,
          isCurrent: true,
          foreignerRegNo: "000115-3******",
        },
        history: [
          { visaId: 90, visaType: "D-4", expireDate: "2024-02-29", status: "만료" },
          { visaId: 80, visaType: "D-4", expireDate: "2023-08-31", status: "만료" }
        ]
      });
      setIsLoading(false);
    }, 300);
  ...
  const status = getStatus(currentVisa.dDay);
  const progress = Math.max(0, Math.min(100, (currentVisa.dDay / 365) * 100));
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Noto Sans KR', sans-serif", fontSize: '14px', color: '#111827' }}>
      <style>{`
        ...
        /* ── 공통 (BasicTab과 동일) ── */
        .vt-topbar {
          background: #fff; padding: 0 28px; height: 58px;
        ...
        .vt-info-key { font-size: 12.5px; color: #9CA3AF; }
        .vt-info-val { font-size: 13px; color: #111827; font-weight: 500; }
        ...
        .vt-dday-left {
          ...
          min-width: 130px; padding-right: 32px; border-right: 1px solid #F3F4F6;
        }
        ...
        .vt-file-box {
          border: 1.5px dashed #E5E7EB; padding: 20px 12px; border-radius: 10px;
          ...
          <div className="vt-dday-field">
            ...
            <div className="vt-dday-field-val" style={{ color: '#1D4ED8' }}>
              ...
              <span style={{ marginLeft: 6 }}>
                <span className="vt-chip vt-chip-blue" style={{ fontSize: '11px' }}>유학</span>
              </span>
            </div>
          </div>
          <div className="vt-dday-field">
            <div className="vt-dday-field-label">외국인 등록번호</div>
            <div className="vt-dday-field-val">{currentVisa.foreignerRegNo}</div>
          </div>
          <div className="vt-dday-field">
        ...
        <div className="vt-card">
          ...
          <div className="vt-timeline">
            <div className="vt-tl-item active">
              <div className="vt-tl-main active">현재 · {currentVisa.visaType} 연장 완료</div>
              <div className="vt-tl-sub">승인일 {currentVisa.issueDate}</div>
            </div>
            {visaData.history.map((h, i) => (
              <div key={i} className="vt-tl-item">
                <div className="vt-tl-main">이전 · {h.visaType}
                  <span className="vt-chip vt-chip-red" style={{ fontSize: '11px', marginLeft: 6 }}>{h.status}</span>
                </div>
