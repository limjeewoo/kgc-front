...
function ConsultTab() {
  ...
  const handleDelete = async (consultId) => {
    if (!window.confirm('정말로 이 상담 기록을 삭제하시겠습니까? 복구할 수 없습니다.')) {
      return;
    }
    try {
      setLoading(true);
      await api.delete(`/api/v1/consultations/${consultId}`);
      alert('상담 기록이 삭제되었습니다.');
      fetchConsultations(searchStudentId);
    } catch (error) {
      console.error('상담 삭제 에러:', error);
      alert(error.response?.data?.message || '상담 기록을 삭제하는 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="consult-tab-container">
      <style>{`
        ...
        /* 기존 width: 32rem (약 512px) -> 45rem (약 720px)로 모달 크기 대폭 확장 */
        .modal-box { background: #fff; border-radius: 1rem; width: 45rem; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1); overflow: hidden; animation: fadeUp 0.2s ease; }
        .modal-hd { padding: 1.5rem; border-bottom: 1px solid #F1F5F9; font-weight: 700; font-size: 1.05rem; color: #0F172A; }
        .modal-bd { padding: 1.5rem; }
        .modal-ft { padding: 1.25rem 1.5rem; background: #F8FAFC; border-top: 1px solid #F1F5F9; display: flex; justify-content: flex-end; gap: 0.75rem; }
        .form-row { display: flex; gap: 1.5rem; margin-bottom: 1.5rem; }
        .form-group { margin-bottom: 1.5rem; flex: 1; }
        .form-lbl { display: block; font-size: 0.8125rem; font-weight: 600; color: #64748B; margin-bottom: 0.5rem; }
        
        /* 모달 확장과 밸런스를 맞추기 위해 인풋창들의 크기와 폰트 규격 확대 */
        .form-input { width: 100%; padding: 0.75rem 1rem; border: 1px solid #E2E8F0; border-radius: 0.5rem; font-size: 0.9rem; outline: none; box-sizing: border-box; }
        .form-input:focus { border-color: #10B981; box-shadow: 0 0 0 3px rgba(16,185,129,0.08); }
        
        /* 브라우저 기본 데이트피커 내부 패딩 및 선택 아이콘 크기 강제 개선 */
        input[type="date"]::-webkit-calendar-picker-indicator {
          transform: scale(1.15);
          cursor: pointer;
        }

        .form-textarea { width: 100%; height: 14rem; padding: 0.75rem 1rem; border: 1px solid #E2E8F0; border-radius: 0.5rem; font-size: 0.9rem; outline: none; box-sizing: border-box; resize: none; font-family: inherit; line-height: 1.6; }
        .form-textarea:focus { border-color: #10B981; box-shadow: 0 0 0 3px rgba(16,185,129,0.08); }
        
        .btn-cancel { background: #E2E8F0; color: #475569; border: none; padding: 0.625rem 1.25rem; border-radius: 0.5rem; font-size: 0.875rem; font-weight: 600; cursor: pointer; }
        .btn-submit { background: #10B981; color: #fff; border: none; padding: 0.625rem 1.25rem; border-radius: 0.5rem; font-size: 0.875rem; font-weight: 600; cursor: pointer; }
            ...
            <div className="consult-item" key={item.consultId || index}>
              <div className="item-meta">
                ...
                <div className="action-group">
                  ...
                  <button className="btn-icon" onClick={() => handleDelete(item.consultId)} title="삭제" style={{ color: '#EF4444' }}>🗑️</button>
                </div>
              </div>
