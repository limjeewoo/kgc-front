...
export default function ProfessorList() {
  ...
  const [editingProfessorId, setEditingProfessorId] = useState(null); // ← 추가
  ...
  const handleOpenRegister = () => {
    setEditingProfessorId(null); // 등록 모드
    setIsRegisterModalOpen(true);
  };

  const handleEdit = (professorId) => {
    setEditingProfessorId(professorId); // 수정 모드
    setIsRegisterModalOpen(true);
  };
  ...
  return (
    <div className="prof-list-container">
      <style>{`
        ...
        .modal-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(15, 23, 42, 0.4); display: flex; align-items: center; justify-content: center; z-index: 999; backdrop-filter: blur(5px); }
        .modal-box { background: #fff; border-radius: 1.25rem; width: 460px; max-width: 90vw; max-height: 85vh; box-shadow: 0 25px 50px -12px rgba(15, 23, 42, 0.25); overflow-y: auto; padding: 2.5rem 2rem; box-sizing: border-box; animation: modalPop 0.25s cubic-bezier(0.16, 1, 0.3, 1); }
        .modal-header-ghost { display: flex; justify-content: flex-end; margin-bottom: -1rem; }
        .modal-close-x { background: none; border: none; font-size: 1.75rem; cursor: pointer; color: #9CA3AF; line-height: 1; padding: 0; }
        .modal-close-x:hover { color: #111827; }
        @keyframes modalPop { from { opacity: 0; transform: scale(0.96) translateY(15px); } to { opacity: 1; transform: scale(1) translateY(0); } }
      `}</style>
      ...
      <div className="table-card">
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="7" className="empty-row">데이터를 불러오는 중입니다...</td></tr>
            ) : filteredProfessors.length === 0 ? (
              <tr><td colSpan="7" className="empty-row">조회된 교수 목록이 없습니다.</td></tr>
            ) : (
