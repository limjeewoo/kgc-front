...
export default function StaffVisaExpirePage() {
  ...
  const [sortKey, setSortKey]     = useState('dDay');   // dDay | name | dept
  const [filterLevel, setFilter]  = useState('ALL');    // ALL | 위험 | 주의
  ...
  const processed = visaList.map(v => ({
    ...v,
    level: getLevel(v.dDay),
  }));

  const filtered = filterLevel === 'ALL' ? processed : processed.filter(v => v.level === filterLevel);
  ...
  return (
    <div style={{ padding:'1.5rem 1.75rem', backgroundColor:'#F0F2F7', minHeight:'100vh', fontFamily:"'DM Sans','Noto Sans KR',sans-serif" }}>
      <style>{`
        .vep-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem; }
        .vep-title  { font-size:1.375rem; font-weight:700; color:#111827; }
        ...
        .vep-filter-btn { padding:6px 14px; border-radius:8px; border:1.5px solid #E5E7EB; background:#fff; color:#6B7280; font-size:0.75rem; font-weight:600; cursor:pointer; font-family:inherit; transition:0.15s; }
        .vep-filter-btn:hover { border-color:#93C5FD; color:#1D4ED8; }
        .vep-filter-btn.active { background:#1A3A5C; color:#fff; border-color:#1A3A5C; }
        .vep-sort { padding:6px 12px; border:1.5px solid #E5E7EB; border-radius:8px; font-size:0.75rem; font-weight:600; background:#fff; color:#374151; cursor:pointer; font-family:inherit; outline:none; margin-left:auto; }
      ...
      <div className="vep-toolbar">
        {['ALL', '위험', '주의'].map(f => (
          <button key={f} className={`vep-filter-btn ${filterLevel === f ? 'active' : ''}`} onClick={() => setFilter(f)}>
            {f === 'ALL' ? `전체 (${processed.length})` : f === '위험' ? `위험 (${dangerCount})` : `주의 (${warningCount})`}
          </button>
        ))}
        <select className="vep-sort" value={sortKey} onChange={e => setSortKey(e.target.value)}>
            ...
            <tbody>
              {sorted.map(v => {
                ...
                return (
                  <tr key={v.studentId}>
                    <td>
                      <span className="vep-level-badge" style={{ background:ls.bg, color:ls.color, borderColor:ls.border }}>
                        {v.level}
                      </span>
                    </td>
                    ...
                    <td>
                      <span style={{ background:'#EFF6FF', color:'#1D4ED8', padding:'2px 8px', borderRadius:6, fontSize:'0.6875rem', fontWeight:700 }}>
                        {v.visaType ?? '-'}
                      </span>
                    </td>
                    <td>{v.expiryDate ?? '-'}</td>
                    <td>
                      <span className="vep-dday" style={{ color: v.level === '위험' ? '#EF4444' : '#D97706' }}>
                        D-{v.dDay}
                      </span>
                    </td>
                  </tr>
