...
export default function StaffDashboard() {
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      let res;
      if (uploadModal === 'student') res = await api.post('/api/v1/students/bulk-upload', fd, { headers:{'Content-Type':'multipart/form-data'} });
      else if (uploadModal === 'foreign') res = await api.post('/api/v1/students/bulk-update-foreign', fd, { headers:{'Content-Type':'multipart/form-data'} });
  ...
  const UPLOAD_META = {
    student: {
      title: '학생 일괄 등록',
      notices: ['1행은 헤더로 건너뛰고 2행부터 데이터를 읽습니다.','학과명은 DB에 등록된 학과명과 정확히 일치해야 합니다.','이미 등록된 학번은 스킵됩니다 (덮어쓰기 안 함).','컬럼 순서를 변경하지 마세요.'],
      columns: '4열: 학번 / 5열: 이름 / 6열: 성별 / 7열: 연락처 / 8열: 학적상태 / 9열: 소속학과 / 11열: 학년 / 12열: 분반',
    },
    foreign: {
      title: '외국인현황 업데이트',
      notices: ['1행은 헤더로 건너뛰고 2행부터 데이터를 읽습니다.','학생정보 엑셀로 학생이 먼저 등록된 상태여야 합니다 (없는 학번은 스킵).','체류자격만료일자는 YYYYMMDD 형식이어야 합니다 (예: 20260930).','주민등록번호/외국인등록번호는 저장되지 않고 비밀번호 설정에만 사용됩니다.','컬럼 순서를 변경하지 마세요.'],
      columns: '4열: 학번 / 6열: 주민등록번호(비번설정용) / 8열: 국적 / 18열: 연락처 / 20열: 체류자격만료일 / 21열: TOPIK급수',
    },
    attend: {
      title: '출결 파일 업로드',
      notices: ['파일명 형식 필수: 과목명 (학과명(N년제)과-학년-반)_attendance.xlsx','예: Java기초 (컴퓨터소프트웨어(3년제)과-2-D)_attendance.xlsx','파일명의 과목명/학과명이 DB에 등록된 것과 정확히 일치해야 합니다.','출석: O / 결석: X / 지각: ▲ / 공결: 빈칸','수강정보가 먼저 등록된 상태여야 합니다.','같은 파일 재업로드 시 기존 출결 데이터를 삭제 후 덮어씁니다.'],
  ...
  const todayFull = `${todayStr} (${days[today.getDay()]})`;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .banner-av { width:52px; height:52px; border-radius:14px; background:rgba(255,255,255,0.15); display:flex; align-items:center; justify-content:center; font-size:1.5rem; flex-shrink:0; }
        .banner-info { flex:1; }
        .banner-name { font-size:1.125rem; font-weight:700; color:#fff; margin-bottom:4px; }
        .banner-sub { font-size:0.75rem; color:rgba(255,255,255,0.6); }
        .banner-stats { display:flex; gap:12px; position:relative; z-index:1; }
        .bstat { background:rgba(255,255,255,0.12); border-radius:12px; padding:10px 16px; text-align:center; cursor:pointer; transition:background 0.15s; border:1px solid rgba(255,255,255,0.1); }
        .bstat:hover { background:rgba(255,255,255,0.2); }
        ...
        .todo-card.t-red::before { background:#EF4444; }
        .todo-card.t-amber::before { background:#F59E0B; }
        .todo-card.t-blue::before { background:#3B82F6; }
        .todo-card:hover { transform:translateY(-2px); box-shadow:0 8px 24px -4px rgba(0,0,0,0.08); border-color:#E2E8F0; }
        .todo-top { display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:12px; }
        .todo-icon { width:36px; height:36px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1.125rem; flex-shrink:0; }
        .todo-badge { font-size:0.6875rem; font-weight:700; padding:3px 10px; border-radius:20px; }
        .todo-badge.red    { background:#FEF2F2; color:#DC2626; }
        .todo-badge.amber  { background:#FFFBEB; color:#D97706; }
        .todo-badge.blue   { background:#EFF6FF; color:#1D4ED8; }
        .todo-badge.gray   { background:#F3F4F6; color:#6B7280; }
        ...
        .dc-hd-title { font-size:0.875rem; font-weight:700; color:#111827; }
        .dc-hd-right { display:flex; align-items:center; gap:8px; }
        .cpill { font-size:0.6875rem; font-weight:700; padding:3px 10px; border-radius:20px; }
        .cpill-red    { background:#FEF2F2; color:#DC2626; }
        .cpill-amber  { background:#FFFBEB; color:#D97706; }
        .cpill-gray   { background:#F3F4F6; color:#6B7280; }
        .cpill-purple { background:#F5F3FF; color:#7C3AED; }

        .sum-body { padding:14px 18px; }
        .sum-row { display:flex; align-items:center; justify-content:space-between; padding:7px 0; border-bottom:1px solid #F9FAFB; }
        .sum-row:last-child { border-bottom:none; }
        .sum-lbl { font-size:0.8125rem; color:#374151; font-weight:500; }
        .sum-badge { font-size:0.6875rem; font-weight:700; padding:3px 10px; border-radius:20px; }
        ...
        .job-av { width:34px; height:34px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:0.875rem; font-weight:700; flex-shrink:0; background:#F5F3FF; color:#7C3AED; }
        ...
        .upload-icon { width:40px; height:40px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1.25rem; flex-shrink:0; }

        @media (max-width:1100px) { .todo-grid { grid-template-columns:1fr 1fr; } .two-grid { grid-template-columns:1fr; } }
      `}</style>
          ...
          <div className="sw-topbar">
            ...
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              ...
              <button onClick={handleLogout}
                style={{ padding:'6px 14px', borderRadius:8, border:'1px solid #E5E7EB', background:'#fff', color:'#374151', fontSize:'0.8125rem', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}
          ...
          {activeMenu === '대시보드' && (
            <div className="sw-content">
              ...
              <div className="staff-banner">
                <div className="banner-av">🗂️</div>
                <div className="banner-info">
                  <div className="banner-name">안녕하세요, 조교님 👋</div>
                  <div className="banner-sub">{todayFull} · {semLabel} 운영 중</div>
                </div>
                <div className="banner-stats">
                  {can('STUDENT_VIEW') && (
                    <div className="bstat" onClick={() => handleMenuClick('학생 목록')}>
                      <div className="bstat-val">{totalStudents}</div>
                      <div className="bstat-lbl">전체 학생</div>
                    </div>
                  )}
                  {can('VISA_VIEW') && (
                    <div className="bstat" onClick={() => handleMenuClick('비자 만료 현황')}>
                      <div className="bstat-val" style={{ color: visaList.length > 0 ? '#FCA5A5' : '#fff' }}>{visaList.length}</div>
                      <div className="bstat-lbl">비자 임박</div>
                    </div>
                  )}
                  {can('ATTEND_VIEW') && (
                    <div className="bstat" onClick={() => handleMenuClick('학과-반별 검색')}>
                      <div className="bstat-val" style={{ color: attendanceList.length > 0 ? '#FCD34D' : '#fff' }}>{attendanceList.length}</div>
                      <div className="bstat-lbl">출결 위험</div>
                    </div>
                  )}
                  {can('JOB_VIEW') && (
                    <div className="bstat" onClick={() => handleMenuClick('근로 승인')}>
                      <div className="bstat-val" style={{ color: pendingJobs.length > 0 ? '#C4B5FD' : '#fff' }}>{pendingJobs.length}</div>
                      <div className="bstat-lbl">승인 대기</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* ── 오늘 할 일 ── */}
              <div className="sec-lbl">오늘 처리할 항목</div>
              <div className="todo-grid" style={{ gridTemplateColumns: `repeat(${[
                can('VISA_VIEW') && visaList.length > 0,
                can('ATTEND_VIEW') && attendanceList.length > 0,
                can('JOB_VIEW') && pendingJobs.length > 0,
              ].filter(Boolean).length || 1}, 1fr)` }}>
                {can('VISA_VIEW') && visaList.length > 0 && (
                  <button className="todo-card t-red" onClick={() => handleMenuClick('비자 만료 현황')}>
                    <div className="todo-top">
                      <div className="todo-icon" style={{ background:'#FEF2F2' }}>🛂</div>
                      <span className="todo-badge red">D-30 이내 {visaDanger.length}명</span>
                    </div>
                    <div className="todo-title">비자 만료 임박</div>
                    <div className="todo-desc">위험 {visaDanger.length}명 · 주의 {visaWarning.length}명 · 확인 필요</div>
                  </button>
                )}
                {can('ATTEND_VIEW') && attendanceList.length > 0 && (
                  <button className="todo-card t-amber" onClick={() => handleMenuClick('학과-반별 검색')}>
                    <div className="todo-top">
                      <div className="todo-icon" style={{ background:'#FFFBEB' }}>⚠️</div>
                      <span className="todo-badge amber">위험 {attendDanger.length}명</span>
                    </div>
                    <div className="todo-title">출결 위험군 발생</div>
                    <div className="todo-desc">위험 {attendDanger.length}명 · 주의 {attendWarning.length}명 · 학과-반별 검색에서 확인</div>
                  </button>
                )}
                {can('JOB_VIEW') && pendingJobs.length > 0 && (
                  <button className="todo-card t-blue" onClick={() => handleMenuClick('근로 승인')}>
                    <div className="todo-top">
                      <div className="todo-icon" style={{ background:'#EFF6FF' }}>📋</div>
                      <span className="todo-badge blue">{pendingJobs.length}건 대기</span>
                    </div>
                    <div className="todo-title">근로 승인 대기</div>
                    <div className="todo-desc">승인 처리가 필요한 근로 신청이 있습니다</div>
                  </button>
                )}
              ...
              {/* ── 알림 현황 ── */}
              <div className="sec-lbl">알림 현황</div>
              <div className="two-grid">
                {can('VISA_VIEW') && (
                  <div className="dc">
                    <div className="dc-hd" onClick={() => handleMenuClick('비자 만료 현황')}>
                      <span className="dc-hd-title">비자 만료 임박</span>
                      <div className="dc-hd-right">
                        {visaDanger.length > 0 && <span className="cpill cpill-red">위험 {visaDanger.length}명</span>}
                        {visaWarning.length > 0 && <span className="cpill cpill-amber">주의 {visaWarning.length}명</span>}
                        {visaList.length === 0 && <span className="cpill cpill-gray">이상 없음</span>}
                    ...
                    <div className="sum-body">
                      {visaList.length === 0 ? (
                        ...
                        <>
                          ...
                          <div className="sum-row"><span className="sum-lbl">주의 (D-31~60)</span><span className="sum-badge sb-amber">{visaWarning.length}명</span></div>
                          <div className="sum-row"><span className="sum-lbl">전체 대상</span><span className="sum-badge sb-gray">{visaList.length}명</span></div>
                        </>
                      )}
                    </div>
                    {visaList.length > 0 && <div className="dc-link" onClick={() => handleMenuClick('비자 만료 현황')}>전체 목록 보기 →</div>}
                  </div>
                )}
                {can('ATTEND_VIEW') && (
                  <div className="dc">
                    <div className="dc-hd" onClick={() => handleMenuClick('학과-반별 검색')}>
                      <span className="dc-hd-title">출결 위험군</span>
                      <div className="dc-hd-right">
                        {attendDanger.length > 0 && <span className="cpill cpill-red">위험 {attendDanger.length}명</span>}
                        {attendWarning.length > 0 && <span className="cpill cpill-amber">주의 {attendWarning.length}명</span>}
                        {attendanceList.length === 0 && <span className="cpill cpill-gray">이상 없음</span>}
                        <span style={{ fontSize:'0.75rem', color:'#9CA3AF' }}>→</span>
                      </div>
                    </div>
                    <div className="sum-body">
                      {attendanceList.length === 0 ? (
                        ...
                        <>
                          ...
                          <div className="sum-row"><span className="sum-lbl">주의 (결석 2~3회)</span><span className="sum-badge sb-amber">{attendWarning.length}명</span></div>
                          <div className="sum-row"><span className="sum-lbl">전체 대상</span><span className="sum-badge sb-gray">{attendanceList.length}명</span></div>
                        </>
                      )}
                    </div>
                    {attendanceList.length > 0 && <div className="dc-link" onClick={() => handleMenuClick('학과-반별 검색')}>출결 현황 보기 →</div>}
                  </div>
                )}
              </div>
              ...
              {can('JOB_VIEW') && (
                <>
                  <div className="sec-lbl">근로 승인 대기</div>
                  <div className="dc" style={{ marginBottom:22 }}>
                    <div className="dc-hd" onClick={() => handleMenuClick('근로 승인')}>
                      <span className="dc-hd-title">승인 처리가 필요한 근로 신청</span>
                      <div className="dc-hd-right">
                        <span className={`cpill ${pendingJobs.length > 0 ? 'cpill-purple' : 'cpill-gray'}`}>{pendingJobs.length}건</span>
                        <span style={{ fontSize:'0.75rem', color:'#9CA3AF' }}>→</span>
                      </div>
                    </div>
                    ...
                    {pendingJobs.length > 5 && <div className="dc-link" onClick={() => handleMenuClick('근로 승인')}>+{pendingJobs.length - 5}건 더 보기 →</div>}
                  </div>
                </>
              )}
              ...
              {(can('STUDENT_UPLOAD') || can('ATTEND_UPLOAD')) && (() => {
                ...
                return (
                  <>
                    <div className="sec-lbl">엑셀 업로드</div>
                    <div className="upload-grid" style={{ gridTemplateColumns:`repeat(${uploadItems.length},1fr)` }}>
                      {uploadItems.map(item => (
                        <button key={item.key} className="upload-card" onClick={() => { setUploadModal(item.key); setUploadFile(null); }}>
                          <div className="upload-icon" style={{ background:item.bg }}>{item.icon}</div>
                          <div>
              ...
              <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, backdropFilter:'blur(2px)' }}
                ...
                <div style={{ background:'#fff', borderRadius:16, width:'36rem', maxWidth:'95vw', boxShadow:'0 20px 40px rgba(0,0,0,0.15)', overflow:'hidden' }}
                  ...
                  <div style={{ padding:'0 1.5rem 1.25rem' }}>
                    <div style={{ border:`2px dashed ${uploadFile ? '#10B981' : '#CBD5E1'}`, borderRadius:12, padding:'2rem', textAlign:'center', cursor:'pointer', background: uploadFile ? '#F0FDF4' : '#F8FAFC', transition:'0.2s' }}
